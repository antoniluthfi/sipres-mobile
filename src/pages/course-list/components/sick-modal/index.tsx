...
const SickModal = forwardRef<BottomSheetModal, PermissionModalProps>(
  ({data}, ref) => {
    const api = useAxios();
    ...
    const handleSubmit = async () => {
      try {
        if (!remarks) {
          Alert.alert('Warning', 'Keterangan harus diisi');
          return;
        }

        if (!file?.length) {
          Alert.alert('Warning', 'File harus diisi');
          return;
        }
    ...
    return (
      <BottomSheetModal
        ...
        <BottomSheetView style={styles.contentContainer}>
          <Text style={styles.title}>Pengajuan Sakit</Text>
